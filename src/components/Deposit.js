  ...
  const handleDeposit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/users/deposit`,
      ...
      setBalances(response.data.balances);
      setSuccess(`Successfully deposited $${parseFloat(amount).toFixed(2)} into your ${accountType} account.`);
      setAmount('');
    } catch (err) {
      if (err.response?.status === 401) {
        localStorage.removeItem('authToken');
    ...
    <div className="container mt-5">
      <div className="card">
        <div className="card-body">
          <h5 className="card-title mb-4">Deposit Funds</h5>
          ...
          <form onSubmit={handleDeposit}>
            ...
            <div className="form-group mb-4">
              <label htmlFor="accountType" className="form-label">To Account</label>
              <select
                id="accountType"
